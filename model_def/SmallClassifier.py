...
class InceptionOne(nn.Module):
    def __init__(self, n0=64, drop = False, p_drop=0.2): 
        ...
        f = n0
        self.first_layer = n0
        self.I1 = Inception(f, 1*f, (1.5*f, 1*f), (0.25*f, 1*f), 1*f, drop = drop, p_drop = p_drop) 
        c2 = f
        self.I2 = Inception(c2, 2*f, (1*f, 2*f), (1*f, 2*f), 2*f, drop = drop, p_drop = p_drop) 
...
class InceptionResidual(nn.Module):
    def __init__(self, cin, n_class, n0=64, \
        ...
        super(InceptionResidual, self).__init__()
...
class SmallResiduals(nn.Module):
    def __init__(self, cin, n_class, n0=64, \
                    dropout=True, p_drop=0.2):
        super(SmallResiduals, self).__init__()
        self.preprocess = FirstConvBlock(cin, 4*n0, dropout, p_drop)
        self.maxpool = nn.MaxPool3d(2)

        self.C1 = Conv(4*n0, 8*n0, dropout=dropout, p_drop=p_drop)
        self.C2 = Conv(8*n0, 8*n0, dropout=dropout, p_drop=p_drop)

        self.C3 = Conv(8*n0, 16*n0, dropout=dropout, p_drop=p_drop)
        self.C4 = Conv(16*n0, 16*n0, dropout=dropout, p_drop=p_drop)

        self.avgpool = nn.AdaptiveAvgPool3d(2)
        self.linear = nn.Sequential(\
                        nn.Linear(int(2**3*16*n0),256),\
                        nn.ReLU(), nn.Linear(256,64),\
                        nn.ReLU(), nn.Linear(64,n_class))
...
class SmallDense(nn.Module):
    def __init__(self, cin, n_class, n0=64, \
        ...
        self.C3 = Conv(12*n0, 16*n0, dropout=dropout, p_drop=p_drop)
        self.C4 = Conv(16*n0, 24*n0, dropout=dropout, p_drop=p_drop)

        self.activate = nn.ReLU()
        self.avgpool = nn.AdaptiveAvgPool3d(2)
        self.linear = nn.Sequential(\
                        nn.Linear(int(8*36*n0),256),\
                        nn.ReLU(), nn.Linear(256,64),\
                        nn.ReLU(), nn.Linear(64,n_class))
